...
> This section requires you to have both Docker and Docker Compose installed. If you want to use our public image, you
> will also need the AWS CLIv2 installed.

We use a modified version of PostgreSQL 15 with the `pgx_ulid` extension. You can use the `packages/pgx-ulid` package to
build the extension into a Docker image, and then run the image to get a PostgreSQL instance with the extension.
