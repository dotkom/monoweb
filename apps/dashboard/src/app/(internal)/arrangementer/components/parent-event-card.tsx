...
import { Button, Group, Image, Loader, Pill, ScrollArea, Stack, Text, Title } from "@mantine/core"
...
const CHILD_EVENT_LIST_MAX_HEIGHT = 338
...
export const ParentEventCard: FC<ParentEventCardProps> = ({
  ...
  if (hasChildren) {
    ...
    return (
      <Stack className={`${classes.card} ${classes.cardWithParent}`} gap="sm">
        <Title order={4} size="sm" fw={500}>
          Dette arrangementet har {childCountLabel}
        </Title>
        <ScrollArea.Autosize mah={CHILD_EVENT_LIST_MAX_HEIGHT} type="auto" offsetScrollbars scrollbarSize={6}>
          <Stack gap="xs">
  ...
  return (
    <Stack className={`${classes.card} ${parentEvent ? classes.cardWithParent : classes.cardWithoutParent}`} gap="sm">
      {parentEvent && (
        <>
          <Title order={4} size="sm" fw={500}>
