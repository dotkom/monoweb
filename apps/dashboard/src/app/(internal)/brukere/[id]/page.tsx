  ...
  return (
    <Box>
      ...
      <Tabs defaultValue={currentTab} onChange={handleTabChange}>
        <Tabs.List>
          {SIDEBAR_LINKS.filter((link) => !link.isAdministrator || isAdministrator).map(({ label, icon: Icon, slug }) => (
            <Tabs.Tab key={slug} value={slug} leftSection={<Icon width={14} height={14} />}>
              {label}
            </Tabs.Tab>
          ))}
        </Tabs.List>
        {SIDEBAR_LINKS.filter((link) => !link.isAdministrator || isAdministrator).map(({ slug, component: Component }) => (
