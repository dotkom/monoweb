...
export interface NotificationRecipientQueryRepository {
  ...
  findUserSummaries(
    handle: DBHandle,
    userIds: UserId[],
    options?: { nameContains?: string; take?: number; cursor?: string }
  ): Promise<RecipientUserSummary[]>
}
...
export function getNotificationRecipientQueryRepository(): NotificationRecipientQueryRepository {
  return {
    ...
    async findUserSummaries(handle, userIds, options) {
      if (userIds.length === 0) {
        return []
      }
      ...
      const nameContains = options?.nameContains?.trim() ?? ""
