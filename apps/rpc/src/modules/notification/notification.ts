...
export const NotificationRecipientReservationStatusSchema = z.enum(["ALL", "RESERVED", "UNRESERVED"])
export type NotificationRecipientReservationStatus = z.infer<typeof NotificationRecipientReservationStatusSchema>

export const NotificationRecipientPaymentStatusSchema = z.enum(["ALL", "PAID", "UNPAID"])
export type NotificationRecipientPaymentStatus = z.infer<typeof NotificationRecipientPaymentStatusSchema>

export const NotificationRecipientAttendanceSelectionOptionSchema = z.object({
  selectionId: z.string(),
  optionId: z.string(),
})
export type NotificationRecipientAttendanceSelectionOption = z.infer<
  typeof NotificationRecipientAttendanceSelectionOptionSchema
>

export const NotificationRecipientMembershipStatusSchema = z.enum(["ACTIVE", "ALL"])
export type NotificationRecipientMembershipStatus = z.infer<typeof NotificationRecipientMembershipStatusSchema>

export const NotificationRecipientMembershipTypeSchema = z.enum([
  "BACHELOR_STUDENT",
  "MASTER_STUDENT",
  "SOCIAL_MEMBER",
  "KNIGHT",
...
export type NotificationRecipientMembershipType = z.infer<typeof NotificationRecipientMembershipTypeSchema>

export const NotificationRecipientSelectionRuleSchema = z.discriminatedUnion("type", [
  z.object({
    type: z.literal("ALL_USERS"),
    membershipStatus: NotificationRecipientMembershipStatusSchema.default("ACTIVE"),
    membershipTypes: z.array(NotificationRecipientMembershipTypeSchema).min(1).optional(),
    studyGrades: z.array(z.number().int().min(1).max(5)).min(1).optional(),
    requiresActiveCommitteeMembership: z.boolean().default(false),
  }),
  z.object({
  ...
  z.object({
    ...
    reservationStatus: NotificationRecipientReservationStatusSchema.default("RESERVED"),
    paymentStatus: NotificationRecipientPaymentStatusSchema.default("ALL"),
    attendanceSelectionOptions: z.array(NotificationRecipientAttendanceSelectionOptionSchema).min(1).optional(),
  }),
])

export type NotificationRecipientSelectionRule = z.infer<typeof NotificationRecipientSelectionRuleSchema>
export type NotificationRecipientSelectionRuleType = NotificationRecipientSelectionRule["type"]

export const NotificationRecipientSelectionSchema = z.object({
  rules: z.array(NotificationRecipientSelectionRuleSchema).min(1, "Minst én mottakerregel er påkrevd"),
  excludedUserIds: z.array(z.string()).default([]),
})

/**
 * Rules for selecting who should and should not receive a notification.
 *
 * Each resolution materializes a point-in-time recipient snapshot. It is not re-evaluated automatically for people
 * who later join a selected group or event.
 */
export type NotificationRecipientSelection = z.infer<typeof NotificationRecipientSelectionSchema>

export const ResolvedNotificationRecipientSchema = z.object({
  userId: z.string(),
  sourceLabels: z.array(z.string()).min(1),
})

export type ResolvedNotificationRecipient = z.infer<typeof ResolvedNotificationRecipientSchema>

export const NotificationRecipientResolutionSchema = z.object({
  /** Users who will receive the notification, after exclusions and preferences. */
  recipients: z.array(ResolvedNotificationRecipientSchema),
...
export type NotificationRecipientResolution = z.infer<typeof NotificationRecipientResolutionSchema>

export const NotificationRecipientSelectionPreviewSchema = NotificationRecipientResolutionSchema.omit({
  recipients: true,
}).extend({
  recipientCount: z.number().int().nonnegative(),
  sample: z.array(
    ResolvedNotificationRecipientSchema.extend({
...
export type NotificationRecipientSelectionPreview = z.infer<typeof NotificationRecipientSelectionPreviewSchema>
...
/**
 * Notification with its initial recipient selection.
 */
export const NotificationManagementSchema = NotificationSchema.extend({
  initialRecipientSelection: NotificationRecipientSelectionSchema.nullable(),
})
...
export const NotificationWriteSchema = NotificationSchema.pick({
  ...
  initialRecipientSelection: NotificationRecipientSelectionSchema,
})
