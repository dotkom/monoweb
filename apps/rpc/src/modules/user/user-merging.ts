...
const CUSTOM_RELATION_MERGERS = {
  userFlagLinks: async (handle: DBHandle, _dependencies: MergeUsersDependencies, survivor: User, consumed: User) => {
    ...
    const idsToTransfer = consumedLinks
      .filter((link) => !survivorKeys.has(toKey(link.userFlagId, link.awardedAt)))
      .map((link) => link.id)
    ...
    // Remaining consumed links are duplicates (same flag and same date), so cascade delete handles them when consumed
    // user is deleted.
  },
