...
export function ReceiptForm() {
  ...
  async function generatePdf(formData: ApiFormData) {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/generate_pdf`, {
      method: "POST",
      headers: {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    alertFormSubmission(`${values.name} har sendt inn kvitteringsskjema med ${values.attachments.length} filer`)
    try {
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto py-10">
      ...
      <Button
        variant="outline"
        className={clsx("mx-auto", formIsEmpty(form.getValues()) ? "hidden" : "")}
        onClick={() => {
          form.setValue("name", "")
          form.setValue("email", "")
        ...
        >
          ...
          <div className="grid grid-cols-12 gap-4 bg-gray-100 p-4 rounded-lg">
            <div className="col-span-12 text-sm text-gray-500">
              Kun en av disse skal fylles ut. Hvis du har betalt for noe med ett av Online sine kort, fyll ut til høyre.
              Hvis du har lagt ut privat, fyll inn kontonr i input til venstre. kontonr som pengene skal gå til.
            </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="F.eks. 1234 56 78901" type="string" {...field} />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Kortnummer/hvilken komite kortet tilhører" type="string" {...field} />
                    </FormControl>
              ...
              <FormItem>
                ...
                <FormControl>
                  <FileUploader
                    ...
                    onValueChange={(files) => {
                      setFiles(files)
                      form.setValue("attachments", files?.map((file) => file.url) ?? [])
                    }}
                ...
                <FormDescription>Du kan laste opp rett fra iphone</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
