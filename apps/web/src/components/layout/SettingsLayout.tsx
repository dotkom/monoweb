...
export const SettingsLayout: FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="w-full border-b-[1px] border-slate-400 py-6 px-4 ">
        <p className=" text-4xl font-bold">{currentLink?.title}</p>
        <p className="text-slate-800 ">{currentLink?.description}</p>
      </div>
      <div className="flex w-full justify-center md:space-x-6 mt-4">
        <SettingsMenuContainer />
        <div className="max-md:w-full w-3/5 max-md:mx-3 border-2 border-slate-400 relative mb-5 rounded-2xl">
          <MobileMenuContainer />
