...
export const ArticleListItem: FC<ArticleListItemProps> = ({ article, orientation }: ArticleListItemProps) => (
  ...
  >
    ...
    <div className="p-4 justify-between flex flex-col flex-1">
        ...
        {orientation === "horizontal" && (
          <div className="flex flex-wrap gap-2 mt-2 max-sm:hidden">
            {article.tags.map((tag) => (
              <Text key={tag.name} className="text-xs text-black bg-slate-200 dark:bg-slate-500 px-2 py-1 rounded-xl">
                {tag.name}
              </Text>
            ))}
          </div>
      ...
      <p className="text-slate-950 dark:text-slate-500 text-sm mt-1">{formatDate(article.updatedAt)}</p>
    </div>
  </Link>
)
