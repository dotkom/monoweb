...
const NodeCommitteeListItem: FC<nodeCommitteeListItemProps> = (props: nodeCommitteeListItemProps) => (
  <div className="p-4 py-8 text-center m-1 shadow-md rounded-lg border-slate-3 border min-w-[250px] flex flex-col transform transition duration-300 hover:scale-[1.02] animate-fadeIn">
    ...
    <h2 className="!text-3xl border-none !mt-4">
      {props.nodeCommittee.name}
      {/* {!props.nodeCommittee.isActive && <p>[inaktiv]</p>} */}
    </h2>
    <p className="mt-2 mb-6 text-left px-3">{props.nodeCommittee.description}</p>
    <Link className="mt-auto text-xl hover:underline" href={`interest-groups/${props.nodeCommittee.id}`}>
      Les mer
    </Link>
  </div>
)
