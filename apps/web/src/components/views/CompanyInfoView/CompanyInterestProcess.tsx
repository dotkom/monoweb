    ...
    <div className="mx-auto flex flex-col items-center md:flex-wrap md:flex-row md:items-stretch justify-evenly max-w-[1024px] px-16 py-4 lg:flex-nowrap ">
      {steps.map((step, index) => (
        <>
          <div
            key={step}
            className="mb-1 w-36 flex flex-col items-center z-10 py-3"
          >
            <Circle size={700 / 15} color="bg-brand">
              <p className="font-bold text-background">{index + 1}</p>
            </Circle>
            <p className="text-brand text-center text-xl font-semibold mt-9 md:mt-14">
              {step}
            </p>
          </div>
          {index != steps.length - 1 && (
            <svg
              ...
              className={`${
                index % 2 == 0 ? "rotate-180 mt-6" : "-mt-10"
              } -mr-10 -ml-10 hidden lg:block`}
            >
              <path
