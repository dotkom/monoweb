          ...
          <ul className="grid w-[700px] gap-2 p-4 md:grid-cols-2">
            {link.items.map((item) => (
              <NavigationMenuLink asChild key={`${link.title}-${item.title}`}>
                <Link
                  href={item.href}
                  className="group hover:bg-blue-100/80 dark:hover:bg-stone-700/50 [[data-theme=pink]_&]:hover:bg-[#E763C6] select-none space-y-1 rounded-lg p-3 leading-none no-underline outline-hidden transition-colors"
                >
