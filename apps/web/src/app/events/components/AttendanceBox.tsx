...
export const AttendanceBox: FC<Props> = ({ sessionUser, attendance, pools, event }) => {
  ...
  const attendablePool = user && pools.find((a) => a.yearCriteria.includes(user.metadata?.study_start_year ?? 0))
  ...
  return (
    <div className="flex flex-col bg-slate-2 rounded-3xl min-h-64 mb-8 px-4 py-4 gap-3">
      <h2>Påmelding</h2>
      {attendablePool && <AttendanceBoxPool pool={attendablePool} isAttending={Boolean(attendee)} />}
