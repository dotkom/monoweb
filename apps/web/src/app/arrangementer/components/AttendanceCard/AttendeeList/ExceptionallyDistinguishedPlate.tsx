...
import { formatExceptionallyDistinguishedCreatedAtYear } from "./exceptionallyDistinguished"

export function ExceptionallyDistinguishedPlate(props: PlateProps) {
  return (
    <Plate {...props} className="group overflow-hidden rounded-full bg-[rgb(12_74_97)] dark:bg-[rgb(8_46_63)]">
      <div className="pointer-events-none absolute inset-0 left-[calc(33%+var(--spacing)*8)] flex items-center justify-center opacity-[0.13] transition-opacity duration-300 group-hover:opacity-25">
...
function getExceptionallyDistinguishedTooltipText(flags: UserFlag[]) {
  const years = flags.map(formatExceptionallyDistinguishedCreatedAtYear)

  return `Særskilt utmerkelse ${ogJoin(years)}`
}

export function getExceptionallyDistinguishedLargeIcon(flags: UserFlag[]) {
  const triggerClassName =
    "relative flex size-10 items-center justify-center overflow-hidden rounded-full bg-white p-px leading-none"
  ...
  const triggerContent = flag.imageUrl ? (
    <Image
      src={flag.imageUrl}
  ...
  return (
    <Tooltip key={FlagNameSchema.enum.EXCEPTIONALLY_DISTINGUISHED} delayDuration={0}>
      <TooltipTrigger className={triggerClassName}>{triggerContent}</TooltipTrigger>
      <TooltipContent>
        <Text>{getExceptionallyDistinguishedTooltipText(flags)}</Text>
      </TooltipContent>
    </Tooltip>
  )
}

export function getExceptionallyDistinguishedSmallIcon(flags: UserFlag[]) {
  const triggerClassName = "relative -my-1 size-5 overflow-hidden rounded-full bg-white p-px"
  const imageUrl = flags[0]?.imageUrl
