...
export function KnightPlate(props: PlateProps) {
  return (
    <Plate
      {...props}
      className={cn(
        "group items-stretch overflow-hidden rounded-lg p-px transition-all duration-300",
    ...
    >
      ...
      />
      <KnightPlateEffects />
      <div className="pointer-events-none relative z-10 flex min-w-0 w-full flex-1 items-center overflow-hidden rounded-[7px] bg-[linear-gradient(to_right,#0d1f4f_0%,#0060a3_68%,#0d1f4f_100%)]">
        <Plate.IdentityArea className="px-2 py-[5px]">
          <Plate.Avatar
            className="relative shadow-[0_0_0_2px_rgba(255,255,255,.15)]"
            fallbackClassName="bg-white/25 text-white"
          />
          <Plate.AttendeeDetails nameClassName="font-medium text-accent" gradeClassName="text-white" />
        </Plate.IdentityArea>
        <KnightPlatePatternArea>
          <Plate.AccessoryArea>
            <Plate.BigIcon className="pr-2" />
          </Plate.AccessoryArea>
        </KnightPlatePatternArea>
      </div>
    </Plate>
...
function KnightPlatePatternArea({ children }: PropsWithChildren) {
  return (
    <div className="pointer-events-none relative flex min-w-4 flex-1 self-stretch overflow-hidden">
