...
export function CourseSectionCard({ title, action, children, className }: Props) {
  return (
    <Card
      className={cn(
        "flex h-full flex-col gap-4 sm:gap-6 rounded-lg p-0 dark:bg-stone-800 border dark:border-stone-700 ring-0",
        className
      )}
    >
      <CardHeader className="flex min-h-11 sm:min-h-14 flex-row items-center border-b border-gray-200 dark:border-stone-700 pl-4 pr-1.5 sm:pl-6 sm:pr-3 py-0!">
