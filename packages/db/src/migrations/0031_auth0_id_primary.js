...
export async function up(db) {
  ...
  await db.schema.alterTable("ow_user").addPrimaryKeyConstraint("ow_user_pkey", ["auth0_id"]).execute()

  // drop the id column
  await db.schema.alterTable("ow_user").dropColumn("id").execute()
...
export async function down(db) {
  for (const [table, constraint, column] of owUserIdForeignKeys) {
    ...
    await db.schema.alterTable(table).addColumn(column, sql`ulid`).execute()
  }
