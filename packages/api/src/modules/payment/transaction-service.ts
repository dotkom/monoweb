import { Product, User } from "@dotkomonline/types"

import { EventRepository } from "../event/event-repository"
import { ProductRepository } from "./product-repository"
import { TransactionRepository } from "./transaction-repository"
import { getStripeObject } from "./../../lib/stripe"
...
export class TransactionServiceImpl implements TransactionService {
  ...
  async fullfillStripeCheckoutSession(stripeSessionId: string): Promise<void> {
    await this.transactionRepository.updateByPaymentProviderOrderId(stripeSessionId, {
      status: "PAID",
    })
  }
  ...
  async expireStripeCheckoutSession(stripeSessionId: string): Promise<void> {
    ...
    await this.transactionRepository.deleteByPaymentProviderOrderId(stripeSessionId)
  }
}
