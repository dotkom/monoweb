...
import { _AttendanceRepositoryImpl } from "../../attendance/repositories/attendance-repository";
...
export const eventPayload: Omit<Event, "id"> = {
  ...
  description:
    "Kotlin er et relativt nytt programmeringsspråk som de siste årene har blitt veldig populært",
...
describe("EventService", () => {
  ...
  const attendanceRepository = new _AttendanceRepositoryImpl(db);
  ...
  it("creates a new event", async () => {
    const id = randomUUID();
    vi.spyOn(eventRepository, "create").mockResolvedValueOnce({
      id,
      ...eventPayload,
    });
  ...
  it("finds events by id", async () => {
    ...
    vi.spyOn(eventRepository, "getById").mockResolvedValueOnce({
      id,
      ...eventPayload,
    });
