...
export function prepareJsonInsert<T extends object>(
  obj: T,
  key: keyof T
...
export function orderedQuery<DB, TB extends keyof DB, O>(
...
export const paginatedQuery = async <
  T extends OrderedIdentifier,
  DB,
  TB extends keyof DB,
  O extends OrderedIdentifier
>(
  qb: SelectQueryBuilder<DB, TB, O>,
  pageable: Pageable,
  mapper: (payload: O) => T
  ...
  const hasNextCursor =
    records.length !== 0 && records.length === pageWithNextLength;
