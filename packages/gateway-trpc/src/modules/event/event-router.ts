...
export const eventRouter = t.router({
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const committees = await ctx.eventCommitteeService.setEventCommittees(
        event.id,
        input.committeeIds,
      );
      return {
  ...
  editWithCommittees: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const event = await ctx.eventService.updateEvent(input.id, input.event);
      await ctx.eventCommitteeService.setEventCommittees(
        input.id,
        input.committees,
      );
      return event;
  ...
  all: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const committees = events.map(async (e) =>
        ctx.eventCommitteeService.getEventCommitteesForEvent(e.id),
      );
  ...
  allByCompany: publicProcedure
    .input(
      z.object({ id: CompanySchema.shape.id, paginate: PaginateInputSchema }),
    )
    .query(async ({ input, ctx }) =>
      ctx.companyEventService.getEventsByCompanyId(
        input.id,
        input.paginate.take,
        input.paginate.cursor,
      ),
    ),
  ...
  allByCommittee: publicProcedure
    ...
    .query(async ({ input, ctx }) =>
      ctx.eventService.getEventsByCommitteeId(
        input.id,
        input.paginate.take,
        input.paginate.cursor,
      ),
  ...
  get: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const committees =
        await ctx.eventCommitteeService.getEventCommitteesForEvent(event.id);
